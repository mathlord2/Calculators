...
eqCaltAB = sympy.Eq(xSlopeAB * (x - c[0]) + ySlopeAB * (y - c[1]), 0)
#Calculates equation of altitude from C to AB

eqAaltBC = sympy.Eq(xSlopeBC * (x - a[0]) + ySlopeBC * (y - a[1]), 0)
...
print("Equation 1: " + str(sympy.simplify(eqCaltAB)))
print("Equation 2: " + str(sympy.simplify(eqAaltBC)))

print("The coordinates of the Triangle's Orthocentre are:\n " + str(sympy.linsolve([eqCaltAB, eqAaltBC], (x, y))))
